...
TOOLCHAIN_PATH=$HOME/ReClang/bin
...
if [ -z "$1" ]; then
    ...
    echo "    bash sign.sh lmi"
    echo "Build for umi(Mi10) with KernelSU:"
    echo "    bash sign.sh umi ksu"
    exit 1
fi
...
echo "Clone AnyKernel3 for packing kernel (repo: https://github.com/SaleOff/AnyKernel3)"
git clone https://github.com/SaleOff/AnyKernel3 -b kona --single-branch --depth=1 anykernel

# Add date to local version.
local_version_str="-perf"
local_version_date_str="-ꑄℭѧᝰ.ᐟ-${GIT_COMMIT_ID}-perf"
...
ZIP_FILENAME=${KSU_ZIP_STR}_${TARGET_DEVICE}_ꑄℭѧᝰ.ᐟ.zip
